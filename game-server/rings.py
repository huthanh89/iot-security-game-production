...
class Mission():
    ...
    def deserialize(self, data):
        ...
        self.done = data["done"]
        self.quizCorrectCount = data["quizCorrectCount"]
        self.pointsEarned = data["pointsEarned"]

        if data["state"] is not None:
            self.state = self.fsm[data["state"]]
    ...
    def sendStateDataToPlayer(self, player=None):
        ...
        text = text.replace("[[device]]", self.deviceDns
                                                                                        ...
                                                                                        self.player.name
                                                                                        ).replace("[[disableAnswers]]",
                                                                                                  "true" if self.player != player else "false"
                                                                                                  ).replace(
            "[[otherPlayerInProgress]]", (self.player.name + " is working on it.") if self.player != player else ""
            )
...
class TeamGameData():
    ...
    def serialize(self):
        ...
        for missionId, mission in self.missions.items():
            data["missions"][missionId] = mission.toJSON()

        data["levels"] = []
        for i, level in enumerate(self.levels):
            levelData = {}
            for missionId in level:
                levelData[missionId] = "<mission_pointer_see_id>"
            data["levels"].append(levelData)
        '''
        #		for levelId in self.levels:
        #	data["levels"][levelId] = self.levels[levelId].toJSON()
        return data

    def deserialize(self, data):

        self.currentRing = data["currentRing"]
        self.progress = data["progress"]
        self.done = data["done"]

        for id, mission in self.missions.items():
            mission.deserialize(data["missions"][id])


        '''
        for index, ring in enumerate(self.model["missions"]):
            level = {}
            for missionId in ring:
                missionModel = ring[missionId]
                missionModel["level"] = index
                ...
                mission.deserialize(data["missions"][missionId])
...
class Rings():
    ...
    def __init__(self, teams, players, devices):
        ...
        self.teamConfig = None
        self.otherConfig = None

    def setup(self, level, teamConfig, otherConfig, sendGameStateToAll, fromAutoSave):
    ...
    def serialize(self):
        ...
        for team in self.teams:
            data["teams"].append(team.serialize())

        for playerId, player in self.players.items():
            data["players"][playerId] = player.serialize()

        for team, teamData in self.teamData.items():
            data["teamData"][team.name] = teamData.serialize()

        data["teamConfig"] = self.teamConfig
        data["otherConfig"] = self.otherConfig
        return data

    def deserialize(self, data):

        print("rings.deserialize() " + str(len(data["teams"])))
        ...
        self.teamConfig = data["teamConfig"]
        self.otherConfig = data["otherConfig"]

        for teamData in data["teams"]:
            team = Team("")
            team.deserialize(teamData)
            self.teams.append(team)
        ...
        for teamId, teamData in data["teamData"].items():
            current_team = None
            for team in self.teams:
                if team.name == teamId:
                    current_team = team
                    break
            teamDataObject = TeamGameData(self.level, current_team)
            teamDataObject.deserialize(teamData)
            self.teamData[current_team] = teamDataObject


        '''
        for index, config in enumerate(self.teamConfig):
            team = None
            for teamSearch in self.teams:
                if teamSearch.name == config["name"]:
                    team = teamSearch
